...
import { 
  ...
  Search, Layers, MoreVertical, Sparkles, Loader2,
  Wand2
} from 'lucide-react';

/**
 * DEVELOPER NOTE: Using React for Chrome Extensions
 * When deploying this to a real extension:
 * 1. Build the project (npm run build).
 * 2. Ensure your manifest.json points to the generated index.html.
 * 3. Content Security Policy (CSP) in manifest.json must allow 
 * the scripts generated by your build tool (like Vite or Webpack).
 */
...
export default function App() {
  ...
  const [activeView, setActiveView] = useState('dashboard'); // dashboard | editor
  const [editingListId, setEditingListId] = useState(null);
  const [isExtracting, setIsExtracting] = useState(false);

  useEffect(() => {
    // Chrome Extension Specific Loading logic
  ...
  useEffect(() => {
    // Persistent Syncing
    if (typeof chrome !== 'undefined' && chrome.storage && chrome.storage.local) {
      chrome.storage.local.set({ 'highlighter_lists_v3': lists });
    }
  ...
  // AI Feature: Auto-Extract from Page
  const handleAutoExtract = async () => {
    setIsExtracting(true);
    try {
      const prompt = "Analyze common web patterns and suggest 10 high-value technical keywords to highlight. Return ONLY a comma-separated list of words.";
      const response = await callGemini(prompt, "You are a professional web data analyst.");
  ...
  return (
    <div className="min-h-screen bg-[#020617] text-slate-200 font-sans selection:bg-indigo-500/30">
      <div className="max-w-[480px] mx-auto min-h-screen flex flex-col bg-[#020617] border-x border-slate-800/50 shadow-2xl">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <div className="flex items-center gap-1.5">
                ...
                <span className="text-[10px] text-slate-500 font-medium tracking-tight uppercase">React Extension Mode</span>
              </div>
            </div>
          </div>
...
function Dashboard({ lists, onCreate, onEdit, onDelete, onToggle, onExtract, isExtracting }) {
  return (
    <div className="p-6 space-y-8 animate-in fade-in duration-500">
      <div className="flex justify-between items-start">
        <div>
          <h2 className="text-xl font-semibold text-white">Library</h2>
          <p className="text-xs text-slate-500 mt-1">React-powered state management</p>
        </div>
        <div className="flex flex-col gap-2">
          ...
          <button 
            ...
            className="flex items-center justify-center gap-2 bg-slate-800 hover:bg-slate-700 disabled:opacity-50 text-indigo-400 px-4 py-2 rounded-lg transition-all font-semibold text-xs uppercase tracking-wider border border-slate-700"
          >
        ...
        <div className="grid gap-3">
          {lists.map(list => (
            ...
            >
              ...
              <div className="flex-1 min-w-0" onClick={() => onEdit(list.id)}>
                <div className="flex items-center gap-2">
                   <h3 className="font-medium text-sm text-slate-200 truncate cursor-pointer">{list.name}</h3>
...
function ListEditor({ list, onChange, onBack }) {
  ...
  const handleSmartSuggest = async () => {
    ...
    try {
      const prompt = `Based on the list title "${list.name}" and existing words [${list.words.join(', ')}], suggest 5 more highly relevant words to highlight. Return ONLY a comma-separated list.`;
      const response = await callGemini(prompt, "You are a helpful language assistant.");
      if (response) {
        const suggestions = response.split(',').map(s => s.trim()).filter(s => s);
        onChange({ words: [...new Set([...list.words, ...suggestions])] });
      }
    } catch (err) {
    ...
    <div className="space-y-8 animate-in slide-in-from-bottom-4 duration-400">
      ...
      <div>
        ...
        <div className="relative group">
          ...
          <button 
            ...
            className="absolute right-0 bottom-3 text-indigo-400 hover:text-indigo-300 disabled:opacity-50"
      ...
      <div className="space-y-4">
        <div className="flex justify-between items-center">
            ...
            <button 
                onClick={handleSmartSuggest}
                className="text-[10px] text-indigo-400 font-bold uppercase hover:text-indigo-300 transition-colors flex items-center gap-1"
            >
                <Sparkles size={10} /> ✨ Smart Fill
            </button>
        </div>
